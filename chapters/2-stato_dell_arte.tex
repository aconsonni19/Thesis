...
potrebbe contenere una debolezza software oppure un problema di implementazione.
Questa tecnica viene effettuata attraverso appositi programmi, detti \textit{fuzzers}, come per esempio \textbf{American Fuzzy Lop} (AFL) .
...
\end{itemize}
L'analisi tramite fuzzing è facilmente implementabile, facilmente estensibile, scalabile e non richiede all'analista una conoscenza approfondita del programma sotto test. Tuttavia, la sua efficienza dipende da una serie di fattori, come per esempio
la metodologia di generazione e mutazione dell'input: per effettuare una buona analisi di sicurezza del programma, è necessario che gli input generati siano in grado di passare i controlli di sanificazione che il programma effettua sull'input;  
...
possibile di cammini di esecuzione interessanti. Diventa quindi necessario capire come \textbf{guidare la generazione dell'input}: 
